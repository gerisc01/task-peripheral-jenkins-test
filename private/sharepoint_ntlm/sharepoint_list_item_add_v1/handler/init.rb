...
class SharepointListItemAddV1
  ...
  def execute()
    ...
    httpget = HttpGet.new("#{homepage_url.chomp("/")}/_api/web/lists/GetByTitle('#{URI.encode(@parameters['list_name'])}')/ListItemEntityTypeFullName")
    httpget.setHeader("Accept","application/json; odata=verbose")

    response = client.execute(target, httpget, context)
    check_for_sp_error(response)
    typeString =  EntityUtils.toString(response.getEntity())
    ...
    type = JSON.parse(typeString)["d"]["ListItemEntityTypeFullName"]
    ...
    httppost = HttpPost.new("#{homepage_url.chomp("/")}/_api/web/lists/GetByTitle('#{URI.encode(@parameters['list_name'])}')/items")
    httppost.setHeader("Accept","application/json; odata=verbose")
    httppost.setHeader("Content-Type","application/json; odata=verbose")
    httppost.setHeader("X-RequestDigest",form_digest_value)
    httppost.setEntity(StringEntity.new(list_item.to_json))
